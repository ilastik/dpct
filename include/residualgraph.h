...
class ResidualGraph : public lemon::ListDigraph {
  public: // typedefs
    ...
    struct ResidualArcProperties {
        ...
        ResidualArcProperties(double c = std::numeric_limits<double>::infinity(), bool e = true, bool p = true,
                              Arc a = lemon::INVALID)
  ...
  public: // API
    ResidualGraph(const Graph &original, const OriginalNode &origSource,
                  const std::map<OriginalNode, size_t> &nodeTimestepMap, bool useBackArcs = true,
    ...
    ShortestPathResult findShortestPath(const std::vector<OriginalNode> &origTargets, bool partialBFUpdates = true);
  ...
  private:
    ...
    /**
     * @brief Transform an arc of the original graph into an forward oriented node pair (the ResidualArcCandidate) of
     * the residual graph
     * @param a original graph arc
     *
    ...
    /**
     * @brief Transform an arc of the original graph into an backward oriented node pair (the ResidualArcCandidate) of
     * the residual graph
     * @param a original graph arc
     *
    ...
    /**
     * @brief Transform an arc of the residual graph into an ordered node pair (the ResidualArcCandidate) of the
     * residual graph
     * @param a residual graph arc
     *
     * @return arc in residual graph encoded as pair of nodes
     */
    ResidualArcCandidate residualArcToPair(const Arc &a) const { return std::make_pair(source(a), target(a)); }
     ...
     */
    Arc pairToResidualArc(const ResidualArcCandidate &ac) const { return lemon::findArc(*this, ac.first, ac.second); }
...
inline void ResidualGraph::updateResidualArc(const ResidualArcCandidate &ac, double cost, int capacity) {
    DEBUG_MSG("Updating residual arc (" << id(ac.first) << ", " << id(ac.second) << ") with cost " << cost
