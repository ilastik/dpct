...
class Arc : public IUserDataHolder, public IArcNotifier {
  public:
    enum Type { Appearance, Disappearance, Move, Division, Swap, Dummy };
  ...
  public:
    ...
    Arc(Node *source, Node *target, Type type, const std::vector<double> &scoreDeltas,
        Node *dependsOnCellInNode = nullptr, UserDataPtr data = UserDataPtr());

    Arc(const Arc &a, NodeMapFunc map_node, UserDataPtr data = UserDataPtr());
