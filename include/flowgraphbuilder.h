...
class FlowGraphBuilder : public GraphBuilder {
  public:
    FlowGraphBuilder(FlowGraph *graph) : graph_(graph) {}
    ...
    FlowGraph::Arc getAppearanceArc(size_t nodeId) {
        for (FlowGraph::Graph::InArcIt ia(graph_->getGraph(), idToFlowGraphNodeMap_[nodeId].u); ia != lemon::INVALID;
             ++ia) {
    ...
    FlowGraph::Arc getDisappearanceArc(size_t nodeId) {
        for (FlowGraph::Graph::OutArcIt oa(graph_->getGraph(), idToFlowGraphNodeMap_[nodeId].v); oa != lemon::INVALID;
             ++oa) {
    ...
    FlowGraph::Arc getNodeArc(size_t nodeId) { return idToFlowGraphNodeMap_[nodeId].a; }

    FlowGraph::Arc getMoveArc(std::pair<size_t, size_t> ids) { return idTupleToFlowGraphArcMap_[ids]; }

    std::pair<FlowGraph::Arc, FlowGraph::Arc> getDivisionArcs(size_t parent, size_t child) {
        for (FlowGraph::Graph::OutArcIt oa(graph_->getGraph(),
                                           graph_->getGraph().target(idToFlowGraphDivisionArcMap_[parent]));
             oa != lemon::INVALID; ++oa) {
