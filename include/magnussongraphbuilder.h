...
class MagnussonGraphBuilder : public GraphBuilder {
  public:
    MagnussonGraphBuilder(Graph *graph) : graph_(graph) {}
    ...
    void addNode(size_t id, const CostDeltaVector &detectionCosts, const CostDeltaVector &detectionCostDeltas,
                 const CostDeltaVector &appearanceCostDeltas, const CostDeltaVector &disappearanceCostDeltas,
                 size_t targetIdx = 0) {
        ...
        Graph::NodePtr n = graph_->addNode(timestep, flipSign(detectionCosts), flipSign(appearanceCostDeltas),
                                           flipSign(disappearanceCostDeltas), false, false);
    ...
    std::vector<FlowGraph::Path> translateSolution(const std::vector<TrackingAlgorithm::Path> &paths,
        ...
        for (const TrackingAlgorithm::Path &p : paths) {
            ...
            // a path starts at the dummy-source and goes to the dummy-sink. these arcs are of type dummy, and thus
            // skipped
            ...
            for (const Arc *a : p) {
                ...
                case Arc::Move: {
                    ...
                    if (first_arc_on_path) {
                        flowPath.push_back(std::make_pair(
                            builder.getAppearanceArc(graphNodeToIdMap_[a->getSourceNode()->getSharedPtr()]), 1));
                        flowPath.push_back(std::make_pair(
                            builder.getNodeArc(graphNodeToIdMap_[a->getSourceNode()->getSharedPtr()]), 1));
                    ...
                    flowPath.push_back(std::make_pair(
                        ...
                        1));
                    flowPath.push_back(
                        std::make_pair(builder.getNodeArc(graphNodeToIdMap_[a->getTargetNode()->getSharedPtr()]), 1));
                } break;
                case Arc::Appearance: {
                    // the node that appeared is set active here, so detections without further path are active as well
                    flowPath.push_back(std::make_pair(
                        builder.getAppearanceArc(graphNodeToIdMap_[a->getTargetNode()->getSharedPtr()]), 1));
                    flowPath.push_back(
                        std::make_pair(builder.getNodeArc(graphNodeToIdMap_[a->getTargetNode()->getSharedPtr()]), 1));
                    first_arc_on_path = false;
                ...
                case Arc::Disappearance: {
                    ...
                    first_arc_on_path = false;
                    flowPath.push_back(std::make_pair(
                        builder.getDisappearanceArc(graphNodeToIdMap_[a->getSourceNode()->getSharedPtr()]), 1));
                } break;
                case Arc::Division: {
                    assert(a->getObservedNode() != nullptr);
                    auto flowArcs = builder.getDivisionArcs(graphNodeToIdMap_[a->getObservedNode()->getSharedPtr()],
                                                            graphNodeToIdMap_[a->getTargetNode()->getSharedPtr()]);
            ...
            for (const Arc *a : p) {
                ...
                case Arc::Division: {
                    ...
                    for (Node::ConstArcIt ai = a->getObservedNode()->getOutArcsBegin();
                         ai != a->getObservedNode()->getOutArcsEnd(); ++ai) {
