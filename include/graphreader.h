...
class DPCT_API GraphReader {
  ...
  protected:
    GraphReader(GraphBuilder *graphBuilder) : graphBuilder_(graphBuilder) {}

    FeatureVector weightedSumOfFeatures(const StateFeatureVector &stateFeatures, const FeatureVector &weights,
                                        size_t offset, bool statesShareWeights);
