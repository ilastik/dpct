...
class DPCT_API Graph {
  public:
    ...
    class DPCT_API Configuration {
      public:
        Configuration(bool enableAppearance, bool enableDisappearance, bool enableDivision);
  ...
  public:
    ...
    NodePtr addNode(size_t timestep, const std::vector<double> &cellCountScoreDelta,
                    const std::vector<double> &appearanceScoreDelta = {0.0},
                    const std::vector<double> &disappearanceScoreDelta = {0.0}, bool connectToSource = false,
                    bool connectToSink = false, UserDataPtr data = UserDataPtr());

    // add an arc between used defined nodes. Nodes have to be created first!
    ArcPtr addMoveArc(NodePtr source, NodePtr target, const std::vector<double> &scoreDeltas,
    ...
    ArcPtr allowMitosis(NodePtr parent, NodePtr child, double divisionScoreDelta);
    ArcPtr allowMitosis(Node *parent, Node *child, double divisionScoreDelta);
