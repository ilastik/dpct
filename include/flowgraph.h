...
class DPCT_API FlowGraph {
  ...
  public: // API
     ...
     */
    double maxFlowMinCostTracking(double initialStateEnergy = 0.0, bool useBackArcs = true, size_t maxNumPaths = 0,
                                  bool useOrderedNodeListInBF = true, bool partialBFUpdates = true);
    ...
    /// get one of the targets: if the index > 0 this can lazily add additional target nodes (needed for duplicated
    /// graphs only)
...
inline void FlowGraph::toggleOutArcsBut(const Node &n, const Node &exception, bool state) {
    DEBUG_MSG("Setting out arcs of " << baseGraph_.id(n) << " but " << baseGraph_.id(exception) << " to "
                                     << (state ? "true" : "false"));
    for (Graph::OutArcIt oa(baseGraph_, n); oa != lemon::INVALID; ++oa) {
...
inline void FlowGraph::toggleInArcsBut(const Node &n, const Node &exception, bool state) {
    DEBUG_MSG("Setting in arcs of " << baseGraph_.id(n) << " but " << baseGraph_.id(exception) << " to "
                                    << (state ? "true" : "false"));
    for (Graph::InArcIt ia(baseGraph_, n); ia != lemon::INVALID; ++ia) {
...
inline void FlowGraph::enableArc(const Arc &a, bool state) { residualGraph_->enableArc(a, state); }
 ...
 */
template <class T> std::ostream &operator<<(std::ostream &stream, const std::vector<T> &feats) {
    stream << "(";
