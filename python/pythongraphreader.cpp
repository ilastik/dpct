...
PythonGraphReader::PythonGraphReader(boost::python::dict &graphDict, boost::python::dict &weightsDict,
                                     GraphBuilder *graphBuilder)
    : GraphReader(graphBuilder), graphDict_(graphDict), weightsDict_(weightsDict) {}

size_t PythonGraphReader::getNumWeights(boost::python::dict &hypothesis, GraphReader::JsonTypes type,
...
void PythonGraphReader::createGraphFromPython() {
    ...
    if (graphDict_.has_key(GraphReader::JsonTypeNames[GraphReader::JsonTypes::Settings])) {
        ...
        if (settings.has_key(GraphReader::JsonTypeNames[GraphReader::JsonTypes::StatesShareWeights]))
            statesShareWeights =
                extract<bool>(settings[GraphReader::JsonTypeNames[GraphReader::JsonTypes::StatesShareWeights]]);
    }
    ...
    list segmentationHypotheses =
        extract<list>(graphDict_[GraphReader::JsonTypeNames[GraphReader::JsonTypes::Segmentations]]);
    ...
    for (size_t i = 0; (int)i < len(segmentationHypotheses); i++) {
        ...
        if (jsonHyp.has_key(GraphReader::JsonTypeNames[GraphReader::JsonTypes::DisappearanceTarget])) {
            targetIdx =
                extract<size_t>(jsonHyp[GraphReader::JsonTypeNames[GraphReader::JsonTypes::DisappearanceTarget]]);
        }
    ...
    // }
    if (graphDict_.has_key(GraphReader::JsonTypeNames[GraphReader::JsonTypes::Exclusions]) &&
        len(graphDict_[GraphReader::JsonTypeNames[GraphReader::JsonTypes::Exclusions]]) > 0)
...
PythonGraphReader::StateFeatureVector PythonGraphReader::extractFeatures(boost::python::dict &entry,
...
PythonGraphReader::FeatureVector PythonGraphReader::readWeightsFromPython(boost::python::dict &weightsDict) {
    list weightsList = extract<list>(
        weightsDict[GraphReader::GraphReader::JsonTypeNames[GraphReader::GraphReader::JsonTypes::Weights]]);
