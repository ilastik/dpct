...
int main(int argc, char **argv) {
    ...
    description.add_options()("help", "produce help message")("model,m", po::value<std::string>(&modelFilename),
                                                              "filename of model stored as Json file")(
        ...
        "filename where the resulting tracking (as links) will be stored as Json file")(
        "method,e", po::value<std::string>(&method),
        "method to use for tracking: 'flow' (default), 'flow-flow', 'magnusson-flow' or 'magnusson'")(
        ...
        "orderNodes", po::value<bool>(&useOrderedNodeListInBF),
        "use ordered node list in BF? flow only. (default=true)")(
        "partialBF", po::value<bool>(&partialBFUpdates),
        "check which parts of the graph were influenced by last path and only update there? flow only. (default=true)");
    ...
    if (!variableMap.count("model") || !variableMap.count("output") || !variableMap.count("weights")) {
        ...
        if (method == "flow") {
            ...
            jsonReader.createGraphFromJson();
            std::cout << "Model has state zero energy: " << jsonReader.getInitialStateEnergy() << std::endl;
            double energy = graph.maxFlowMinCostTracking(jsonReader.getInitialStateEnergy(), false, maxNumPaths,
                                                         useOrderedNodeListInBF, partialBFUpdates);
            ...
            flowGraph.maxFlowMinCostTracking(zeroEnergy - score, true, maxNumPaths, useOrderedNodeListInBF,
                                             partialBFUpdates);
