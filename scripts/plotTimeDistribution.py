...
def parseFile(filename):
    ...
    return (
        np.array(initBfTimes),
        np.array(bfTimes),
        np.array(pathTimes),
        np.array(augmentTimes),
        np.array(constraintTimes),
        np.array(iterationTimes),
    )
...
if __name__ == "__main__":
    ...
    if args.pieOutFile is not None:
        ...
        plt.pie(
            cumulativeTime[:-1], labels=labels[:-1], colors=colors[:4], autopct="%1.1f%%", shadow=True, startangle=90
        )
