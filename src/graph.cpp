...
Graph::Configuration::Configuration(bool enableAppearance, bool enableDisappearance, bool enableDivision)
    : withAppearance(enableAppearance), withDisappearance(enableDisappearance), withDivision(enableDivision) {}

Graph::Graph(const Graph::Configuration &config)
    : config_(config), sourceNode_(std::vector<double>(), std::make_shared<NameData>("Source")),
      sinkNode_(std::vector<double>(), std::make_shared<NameData>("Sink")), numNodes_(0) {}

Graph::NodePtr Graph::addNode(size_t timestep, const std::vector<double> &cellCountScoreDelta,
                              const std::vector<double> &appearanceScoreDelta,
                              const std::vector<double> &disappearanceScoreDelta, bool connectToSource,
                              bool connectToSink, UserDataPtr data) {
    ...
    if (config_.withAppearance && !connectToSource) {
        ArcPtr arc(new Arc(&sourceNode_, node.get(), Arc::Appearance, appearanceScoreDelta, nullptr, nullptr));
        arcs_.push_back(arc);
    }

    // create arc to disappearance if this is not the very last frame
    if (config_.withDisappearance && !connectToSink) {
        ArcPtr arc(new Arc(node.get(), &sinkNode_, Arc::Disappearance, disappearanceScoreDelta, nullptr, nullptr));
        arcs_.push_back(arc);
    }

    // create arc from source if requested
    if (connectToSource) {
        ArcPtr arc(new Arc(&sourceNode_, node.get(), Arc::Dummy, {0.0}, nullptr, nullptr));
        arcs_.push_back(arc);
    }

    // create arc to sink if requested
    if (connectToSink) {
        ArcPtr arc(new Arc(node.get(), &sinkNode_, Arc::Dummy, {0.0}, nullptr, nullptr));
        arcs_.push_back(arc);
    }
...
Graph::ArcPtr Graph::addMoveArc(NodePtr source, NodePtr target, const std::vector<double> &scoreDeltas,
                                UserDataPtr data) {
    // assert(source in nodes_)
    // assert(target in nodes_)

    ArcPtr arc(new Arc(source.get(), target.get(), Arc::Move, scoreDeltas, nullptr, data));

    arcs_.push_back(arc);
    return arc;
}

Graph::ArcPtr Graph::allowMitosis(Node *parent, Node *child, double divisionScoreDelta) {
    ...
    // add division arc
    ArcPtr arc(new Arc(&sourceNode_, child, Arc::Division, {divisionScoreDelta}, parent));

    arcs_.push_back(arc);
    return arc;
}

Graph::ArcPtr Graph::allowMitosis(NodePtr parent, NodePtr child, double divisionScoreDelta) {
    ...
    for (NodeVector nv : nodesPerTimestep_) {
        ...
        for (NodePtr np : nv) {
            ...
            for (Node::ConstArcIt it = np->getInArcsBegin(); it != np->getInArcsEnd(); ++it) {
                LOG_MSG("\tArc from " << (*it)->getSourceNode() << " - "
                                      << std::string(((*it)->isEnabled()) ? "enabled" : "disabled"));
            }
            LOG_MSG("and out Arcs:");
            for (Node::ConstArcIt it = np->getOutArcsBegin(); it != np->getOutArcsEnd(); ++it) {
                LOG_MSG("\tArc to " << (*it)->getTargetNode() << " - "
                                    << std::string(((*it)->isEnabled()) ? "enabled" : "disabled"));
            }
        }
    }
...
void Graph::printToDot(const std::string &filename) const {
    ...
    for (NodeVector nv : nodesPerTimestep_) {
        for (NodePtr np : nv) {
            for (Node::ConstArcIt it = np->getOutArcsBegin(); it != np->getOutArcsEnd(); ++it) {
                out_file << "\t" << (size_t)(*it)->getSourceNode() << " -> " << (size_t)(*it)->getTargetNode()
                         << " [ label=\" " << (*it)->typeAsString() << " \" ];\n"
                         << std::flush;
            }
        }
    }
    ...
    for (Node::ConstArcIt it = sourceNode_.getOutArcsBegin(); it != sourceNode_.getOutArcsEnd(); ++it) {
        out_file << "\t" << (size_t)(*it)->getSourceNode() << " -> " << (size_t)(*it)->getTargetNode() << " [ label=\" "
                 << (*it)->typeAsString() << " \" ];\n"
                 << std::flush;
    }
