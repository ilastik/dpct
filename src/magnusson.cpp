...
Magnusson::Magnusson(Graph *graph, bool withSwap, bool usedArcsScoreZero, bool useFastFirstIter)
    : TrackingAlgorithm(graph), withSwap_(withSwap), usedArcsScoreZero_(usedArcsScoreZero),
      selectorFunction_(selectBestInArc), // globally defined function
      maxNumPaths_(std::numeric_limits<size_t>::max()), useFastFirstIter_(useFastFirstIter) {
    assert(usedArcsScoreZero == true);
}

void Magnusson::setPathStartSelectorFunction(SelectorFunction func) { selectorFunction_ = func; }

void Magnusson::setMotionModelScoreFunction(MotionModelScoreFunction func) { motionModelScoreFunction_ = func; }
...
double Magnusson::track(Solution &paths) {
    ...
    while (paths.size() < maxNumPaths_) {
        ...
        if (scoreDelta < 0) {
            DEBUG_MSG("Path has negative reward, stopping here with a total number of " << paths.size()
                                                                                        << " cells added");
            break;
        }
    ...
    if (motionModelScoreFunction_) {
        ...
        for (Node::ArcIt outArc = n->getOutArcsBegin(); outArc != n->getOutArcsEnd(); ++outArc) {
            if ((*outArc)->getType() == Arc::Move) {
                ...
                Node *targetParam =
                    graph_->isSpecialNode((*outArc)->getTargetNode()) ? nullptr : (*outArc)->getTargetNode();
...
void Magnusson::increaseCellCount(Node *n) {
    ...
    //    if(n->getUserData())
    //    {
    //        DEBUG_MSG("Increasing cell count of node " << *(std::static_pointer_cast<NameData>(n->getUserData())) << "
    //        = " << n);
    //    }
    //    else
    //    {
...
void Magnusson::insertSwapArcsForNewUsedPath(TrackingAlgorithm::Path &p) {
     ...
     * - find alternative incoming arcs of the traversed node, where there are also multiple outgoing arcs of the
     * previous node
...
void Magnusson::insertMoveSwapArcs(Arc *a) {
    ...
    for (Node::ArcIt outIt = source->getOutArcsBegin(); outIt != source->getOutArcsEnd(); ++outIt) {
        ...
        for (Node::ArcIt inIt = target->getInArcsBegin(); inIt != target->getInArcsEnd(); ++inIt) {
            ...
            // BUT: it needs to store a reference to the arc that it would cut, and the cleanup action is performed in
            // cleanUpUsedSwapArcs()
            Arc *arc = new Arc(alternativeSource, alternativeTarget, Arc::Swap, {score}, nullptr,
...
void Magnusson::insertAppearanceSwapArcs(Arc *a) {
    ...
    for (Node::ArcIt inIt = target->getInArcsBegin(); inIt != target->getInArcsEnd(); ++inIt) {
        ...
        for (Node::ArcIt outIt = alternativeSource->getOutArcsBegin(); outIt != alternativeSource->getOutArcsEnd();
             ++outIt) {
            Node *alternativeTarget = (*outIt)->getTargetNode();
            assert(alternativeTarget != nullptr);
            if (alternativeTarget == target || graph_->isSpecialNode(alternativeTarget) ||
                (*outIt)->getType() == Arc::Swap)
                continue;

            // find appearance arc of alternativeTarget
            Arc *alternativeAppearanceArc = nullptr;
            for (Node::ArcIt appIt = alternativeTarget->getInArcsBegin(); appIt != alternativeTarget->getInArcsEnd();
                 ++appIt) {
            ...
            // found a candidate
            double score =
                alternativeAppearanceArc->getScoreDelta() + (*inIt)->getScoreDelta() - a->getPreviousScoreDelta();
        ...
        for (SwapCandidateVector::iterator swapIt = swapArcCandidates.begin(); swapIt != swapArcCandidates.end();
             ++swapIt) {
...
void Magnusson::insertDisappearanceSwapArcs(Arc *a) {
    ...
    */
    ...
    for (Node::ArcIt outIt = source->getOutArcsBegin(); outIt != source->getOutArcsEnd(); ++outIt) {
        ...
        for (Node::ArcIt inIt = alternativeTarget->getInArcsBegin(); inIt != alternativeTarget->getInArcsEnd();
             ++inIt) {
            Node *alternativeSource = (*inIt)->getSourceNode();
            assert(alternativeSource != nullptr);
            if (alternativeSource == source || graph_->isSpecialNode(alternativeSource) ||
                (*inIt)->getType() == Arc::Swap)
                continue;

            // find diappearance arc of alternativeSource
            Arc *alternativeDisappearanceArc = nullptr;
            for (Node::ArcIt appIt = alternativeSource->getOutArcsBegin(); appIt != alternativeSource->getOutArcsEnd();
                 ++appIt) {
            ...
            // found a candidate
            double score =
                alternativeDisappearanceArc->getScoreDelta() + (*outIt)->getScoreDelta() - a->getPreviousScoreDelta();
        ...
        for (SwapCandidateVector::iterator swapIt = swapArcCandidates.begin(); swapIt != swapArcCandidates.end();
            ...
            Arc *arc =
                new Arc(alternativeSource, alternativeTarget, Arc::Swap, {score}, nullptr,
    ...
    while (foundSwapArc) {
        ...
        for (std::vector<Arc *>::reverse_iterator p_it = p.rbegin(); p_it != p.rend(); ++p_it) {
            ...
            if (arc->getType() == Arc::Swap) {
                ...
                Arc *replacementP =
                    std::static_pointer_cast<MagnussonSwapArcUserData>(arc->getUserData())->getReplacementAArc();
                Arc *replacementPath =
                    std::static_pointer_cast<MagnussonSwapArcUserData>(arc->getUserData())->getReplacementBArc();
                assert(arcToRemove != nullptr);
                assert(replacementP != nullptr);
                assert(replacementPath != nullptr);

                DEBUG_MSG("Trying to remove swap arc between " << arc->getSourceNode()->getUserData()->toString()
                                                               << " and "
                                                               << arc->getTargetNode()->getUserData()->toString());
...
#if DEBUG_LOG
                            DEBUG_MSG("updated two paths. removed swap arc between "
                                      << arc->getSourceNode()->getUserData()->toString() << " and "
                                      << arc->getTargetNode()->getUserData()->toString());
                            graph_->print();
#endif
...
// -------------------------------------------------------------------------
Arc *selectBestInArc(Node *n) { return n->getBestInArc(); }
...
Arc *selectSecondBestInArc(Node *n) {
    ...
    std::sort(scoreAndArc.begin(), scoreAndArc.end(), [](const SnA &a, const SnA &b) { return a.first > b.first; });
...
Arc *selectAtRandom(Node *n) {
    ...
    size_t numNonNegative =
        std::count_if(scoreAndArc.begin(), scoreAndArc.end(), [](const SnA &a) { return a.first >= 0.0; });
