...
JsonGraphReader::JsonGraphReader(const std::string &modelFilename, const std::string &weightsFilename,
    ...
    : GraphReader(graphBuilder), modelFilename_(modelFilename), weightsFilename_(weightsFilename) {}
...
void JsonGraphReader::createGraphFromJson() {
    ...
    for (int i = 0; i < (int)segmentationHypotheses.size(); i++) {
        ...
        FeatureVector detCosts = weightedSumOfFeatures(extractFeatures(jsonHyp, JsonTypes::Features), weights,
                                                       detWeightOffset, statesShareWeights);
        ...
        if (jsonHyp.isMember(JsonTypeNames[JsonTypes::AppearanceFeatures]))
            appearanceCostDeltas = costsToScoreDeltas(weightedSumOfFeatures(
                extractFeatures(jsonHyp, JsonTypes::AppearanceFeatures), weights, appWeightOffset, statesShareWeights));

        if (jsonHyp.isMember(JsonTypeNames[JsonTypes::DisappearanceFeatures]))
            disappearanceCostDeltas =
                costsToScoreDeltas(weightedSumOfFeatures(extractFeatures(jsonHyp, JsonTypes::DisappearanceFeatures),
                                                         weights, disWeightOffset, statesShareWeights));
    ...
    for (int i = 0; i < (int)linkingHypotheses.size(); i++) {
        ...
        graphBuilder_->addArc(srcId, destId,
                              costsToScoreDeltas(weightedSumOfFeatures(extractFeatures(jsonHyp, JsonTypes::Features),
    ...
    for (int i = 0; i < (int)segmentationHypotheses.size(); i++) {
        ...
        if (jsonHyp.isMember(JsonTypeNames[JsonTypes::DivisionFeatures])) {
            graphBuilder_->allowMitosis(
                id, costsToScoreDelta(weightedSumOfFeatures(extractFeatures(jsonHyp, JsonTypes::DivisionFeatures),
