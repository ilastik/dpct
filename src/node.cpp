...
Node::Node(const std::vector<double> &cellCountScore, UserDataPtr data)
    : IUserDataHolder(data), appearanceArc_(nullptr), disappearanceArc_(nullptr), cellCount_(0), bestInArc_(nullptr),
      cellCountScore_(cellCountScore), currentScore_(0.0), numActiveDivisions_(0), numUsedMoveInArcs_(0),
    ...
    if (cellCountScore_.size() > 1) {
        ...
        throw std::runtime_error(
            "Node - Constructor: Cannot use a cellCountScoreDelta table with only one state, need at least two!");
    }
}

Node::Node(const Node &n, UserDataPtr data) : Node(n.cellCountScore_, data) {}
...
#ifdef DEBUG_LOG
        if (getUserData()) {
            DEBUG_MSG("Node (" << *(std::static_pointer_cast<NameData>(getUserData())) << ") update: score is now "
                               << currentScore_);
        } else {
...
std::ostream &operator<<(std::ostream &lhs, const Node &rhs) {
    ...
    lhs << "), " << rhs.inArcs_.size() << " in-arcs and " << rhs.outArcs_.size() << " out arcs";
    return lhs;
}
