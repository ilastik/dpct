...
GraphReader::FeatureVector GraphReader::weightedSumOfFeatures(const StateFeatureVector &stateFeatures,
                                                              const FeatureVector &weights, size_t offset,
...
GraphReader::FeatureVector GraphReader::costsToScoreDeltas(const FeatureVector &costs) {
    ...
    for (size_t i = 1; i < costs.size(); i++) {
        ...
        if (i > 1 && result[i - 1] <= result[i - 2])
            std::cout << "Warning: found potentially problematic score setup: " << result[i - 1]
                      << " <= " << result[i - 2] << std::endl;
    }
    return result;
}
