...
ResidualGraph::ResidualGraph(const Graph &original, const OriginalNode &origSource,
                             const std::map<OriginalNode, size_t> &nodeTimestepMap, bool useBackArcs,
                             bool useOrderedNodeListInBF)
    : originalGraph_(original), useBackArcs_(useBackArcs), useOrderedNodeListInBF_(useOrderedNodeListInBF),
      residualDistMap_(*this), nodeUpdateOrderMap_(*this), bfDistMap_(*this), bfPredMap_(*this),
      bf(*this, residualDistMap_, bfProcess_, bfNextProcess_), firstPath_(true) {
...
ResidualGraph::ShortestPathResult ResidualGraph::findShortestPath(const std::vector<OriginalNode> &origTargets,
                                                                  bool partialBFUpdates) {
    typedef std::chrono::time_point<std::chrono::high_resolution_clock> TimePoint;
    DEBUG_MSG("Searching shortest path in graph with " << lemon::countNodes(*this) << " nodes and "
                                                       << lemon::countArcs(*this) << " arcs");
    ...
    do {
        ...
        // BUT: the number of paths found changes, which means we are not finding the same things... (different negative
        // cycles?)
        ...
        if (foundPath) {
            ...
            size_t targetIndex = std::distance(targetDistances.begin(),
                                               std::min_element(targetDistances.begin(), targetDistances.end()));
