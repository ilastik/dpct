...
Arc::Arc(Node *source, Node *target, Type type, const std::vector<double> &scoreDeltas, Node *dependsOnCellInNode,
         UserDataPtr data)
    : IUserDataHolder(data), sourceNode_(source), targetNode_(target), type_(type), scoreDeltas_(scoreDeltas),
      currentScore_(scoreDeltas[0]), enabled_(true), used_(0), dependsOnCellInNode_(dependsOnCellInNode) {
...
Arc::Arc(const Arc &a, NodeMapFunc map_node, UserDataPtr data)
    : Arc(map_node(a.sourceNode_), map_node(a.targetNode_), a.type_, a.scoreDeltas_, map_node(a.dependsOnCellInNode_),
          data) {}
...
void Arc::update() {
    ...
    // updateEnabledState();
    DEBUG_MSG(typeAsString() << "-Arc update: score is now " << currentScore_
                             << " (enabled=" << (enabled_ ? "true" : "false") << ")" << " scoreDelta=" << scoreDeltas_);
}
...
std::string Arc::typeAsString() const {
    ...
    case Dummy:
        return "Dummy";
    default:
        throw std::runtime_error("Cannot return string representation of unknown type");
    }
}
...
void Arc::updateEnabledState() {
    ...
    case Division: {
        ...
        if (used_ > 0) {
            ...
            enabled_ =
                dependsOnCellInNode_->getCellCount() == 1 &&
                (dependsOnCellInNode_->getDisappearanceArc() == nullptr ||
                 dependsOnCellInNode_->getDisappearanceArc()->getUseCount() == 0) &&
                (targetNode_->getAppearanceArc() == nullptr || targetNode_->getAppearanceArc()->getUseCount() == 0) &&
                dependsOnCellInNode_->getNumActiveDivisions() == 0 && dependsOnCellInNode_->getMoveOutArcUsedSum() == 1;
            if (enabled_) {
                ...
                for (Node::ConstArcIt outArcIt = dependsOnCellInNode_->getOutArcsBegin();
                     outArcIt != dependsOnCellInNode_->getOutArcsEnd(); ++outArcIt) {
                    if ((*outArcIt)->getTargetNode() == targetNode_) {
                        if ((*outArcIt)->getUseCount() > 0) {
                            // std::cout << "!!!!!!!!!!!!!!!!!!!!!!!! disabling division because the very same link has
                            // been used as transition!" << std::endl;
                            enabled_ = false;
                        }
                        return;
                    }
    ...
    case Swap: {
        ...
        enabled_ &=
            std::static_pointer_cast<MagnussonSwapArcUserData>(this->getUserData())
                ->getReplacementAArc()
                ->isEnabled() &&
...
void Arc::markUsed(bool used) {
    static auto arcEnabler = [](Arc *a) { a->updateEnabledState(); };
