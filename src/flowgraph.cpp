...
namespace dpct {
FlowGraph::FlowGraph() : flowMap_(baseGraph_), capacityMap_(baseGraph_) {
...
FlowGraph::Arc FlowGraph::addArc(FlowGraph::Node source, FlowGraph::Node target, const CostVector &costs) {
    assert(costs.size() > 0);
...
FlowGraph::Arc FlowGraph::addArc(FlowGraph::FullNode source, FlowGraph::FullNode target, const CostVector &costs) {
    return addArc(source.v, target.u, costs);
}

/// create duplicated parent node for the given node with the given cost
FlowGraph::Arc FlowGraph::allowMitosis(FlowGraph::FullNode parent, double divisionCost) {
...
double FlowGraph::maxFlow() {
    ...
    LOG_MSG("Running min cost max flow on a graph with " << lemon::countNodes(baseGraph_) << " nodes and "
                                                         << lemon::countArcs(baseGraph_) << " edges");
...
/// start the tracking
double FlowGraph::maxFlowMinCostTracking(double initialStateEnergy, bool useBackArcs, size_t maxNumPaths,
                                         bool useOrderedNodeListInBF, bool partialBFUpdates) {
    ...
    do {
        ...
        DEBUG_MSG("\tFound path or cycle" << " of length " << result.first.size() << " of distance " << result.second);

        if (result.second > -0.00000001) {
#ifdef DEBUG_LOG
            printPath(result.first);
            std::cout << std::endl;
#endif
            break;
        }
        ...
        if (result.first.size() > 0 && result.second < 0.0) {
            ...
            DEBUG_MSG("augmenting flow took " << elapsed_seconds1.count() << " and updating constraints took "
...
void FlowGraph::initializeResidualGraph(bool useBackArcs, bool useOrderedNodeListInBF) {
    ...
    residualGraph_ =
        std::make_shared<ResidualGraph>(baseGraph_, source_, nodeTimestepMap_, useBackArcs, useOrderedNodeListInBF);
...
void FlowGraph::synchronizeDivisionDuplicateArcFlows() {
    for (auto coupledNodeIt : parentToDuplicateMap_) {
        for (Graph::OutArcIt oa(baseGraph_, coupledNodeIt.first); oa != lemon::INVALID; ++oa) {
            ...
            for (Graph::OutArcIt duplicateOa(baseGraph_, coupledNodeIt.second); duplicateOa != lemon::INVALID;
                 ++duplicateOa) {
...
void FlowGraph::updateArc(const Arc &a) {
    int flow = flowMap_[a];
    DEBUG_MSG("Found " << flow << " flow along arc " << baseGraph_.id(baseGraph_.source(a)) << "->"
                       << baseGraph_.id(baseGraph_.target(a)));
...
void FlowGraph::updateEnabledArcs(const FlowGraph::Path &p) {
    for (const std::pair<Arc, int> &af : p) {
        ...
        DEBUG_MSG("Updating stuff for " << (forward ? "forward" : "backward") << " edge from " << baseGraph_.id(source)
                                        << " to " << baseGraph_.id(target));
#endif
...
void FlowGraph::updateEnabledArc(const FlowGraph::Arc &a) {
    ...
    DEBUG_MSG("Updating stuff for" << " edge from " << baseGraph_.id(source) << " to " << baseGraph_.id(target));
...
void FlowGraph::printAllFlows() {
    for (Graph::ArcIt a(baseGraph_); a != lemon::INVALID; ++a) {
        DEBUG_MSG("\t(" << baseGraph_.id(baseGraph_.source(a)) << ", " << baseGraph_.id(baseGraph_.target(a))
                        << "): " << flowMap_[a]);
    }
}
