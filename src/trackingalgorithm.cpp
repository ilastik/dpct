...
TrackingAlgorithm::TrackingAlgorithm(Graph *graph) : graph_(graph) {}
...
void TrackingAlgorithm::findNonintersectingBackwardPaths(Node *begin, Node *end, Solution &paths) {
    ...
    // init flows to zero
    VisitorFunction vis = [&](Node *n) { forward_flows[n->getBestInArc()] = 0; };
    breadthFirstSearchVisitor(begin, vis);

    // function to find a path with positive residual flow (recursively)
    std::function<bool(const Node *, const Node *, Path &)> findGoodPath = [&](const Node *begin, const Node *end,
                                                                               Path &p) {
        if (begin == end)
            return true;
    ...
    std::function<bool(const Node *, const Node *, Path &)> findGoodPathFromSource = [&](const Node *begin,
                                                                                         const Node *end, Path &p) {
...
void TrackingAlgorithm::printPath(TrackingAlgorithm::Path &p) {
    ...
    if (p.size() > 0)
        DEBUG_MSG("Path starts at node " << nodeName(p.front()->getSourceNode()) << " with cellCount "
                                         << p.front()->getSourceNode()->getCellCount() << " and score "
                                         << p.front()->getSourceNode()->getCurrentScore());
    else
        return;

    for (Path::iterator it = p.begin(); it != p.end(); ++it) {
        DEBUG_MSG("\t follows " << (*it)->typeAsString() << " arc to node " << nodeName((*it)->getTargetNode())
                                << " enabled= " << ((*it)->isEnabled() ? "yes" : "no") << " used= "
                                << (*it)->getUseCount() << " with cellCount " << (*it)->getTargetNode()->getCellCount()
                                << " and score " << (*it)->getTargetNode()->getCurrentScore());
    }
#endif
}

void TrackingAlgorithm::tic() { startTime_ = std::chrono::high_resolution_clock::now(); }
